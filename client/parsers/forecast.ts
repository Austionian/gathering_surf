...
let wave_height_labels: string[];
let wave_heights: string[];
let wind_speeds: string[];
let wind_directions: string[];
let wind_gusts: string[];
let wave_period: string[];
let temperature: string[];
let dewpoint: string[];
let cloud_cover: string[];
let probability_of_precipitation: string[];
let probability_of_thunder: string[];

type ForecastData = {
  wave_height: string[];
  current_wave_height: string;
  current_wave_direction: string;
  current_wave_period: string;
  wind_speed: string[];
  wind_direction: string[];
  wind_gust: string[];
  wave_period: string[];
  wave_height_labels: string[];
  as_of: string[];
  temperature: string[];
 ...
 */
export function parseForecast(data: ForecastData) {
  ...
  setText(
    "legend-quality",
    QUALITY_MAP[qualities[startingAt] as keyof typeof QUALITY_MAP],
  );
   ...
   * data.
   */
  function updateLegends(x: number) {
    let v;
   ...
   */
  const xTicksCallback = (_value: string, i: number) => {
    const beginning = start === 0 ? dataStartingAt : start;
    if (stepBy === 24) {
  ...
  // @ts-expect-error
  const waveForecast = new Chart(
    document.getElementById("forecast") as HTMLCanvasElement,
    {
      type: "bar",
      plugins: [plugin],
      data: {
   ...
   */
  function setForecastRange(label: string) {
   ...
   */
  const getConfig = (data: string[], color: string | null, label: string) => ({
    type: "bar",
    plugins: [plugin],
    data: {
      labels:
  ...
  function precipitationHover(e: Event) {
